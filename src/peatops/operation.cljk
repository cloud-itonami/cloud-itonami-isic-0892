(ns peatops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (PeatOpsAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the PeatExtractionGovernor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [peatops.governor :as governor]
            [peatops.phase :as phase]
...
(defn build
  ...
  `peatops.store/Store`).
  opts:
    :advisor      -- a `peatops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PeatOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; PeatExtractionGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
