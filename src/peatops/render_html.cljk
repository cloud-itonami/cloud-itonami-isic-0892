...
(def ^:private op-p1 {:actor-id "site-supervisor-1" :actor-role :peat-extraction-operator :phase 1})
(def ^:private op-p3 {:actor-id "site-supervisor-1" :actor-role :peat-extraction-operator :phase 3})
(defn- exec! [actor tid request ctx] (g/run* actor {:request request :context ctx} {:thread-id tid}))
(defn- approve! [actor tid] (g/run* actor {:approval {:status :approved :by "site-supervisor-1"}} {:thread-id tid :resume? true}))

(defn run-demo! []
  (let [db (store/seed-db) actor (op/build db)]
    (exec! actor "t1" {:op :log-extraction-record :site-id "peat-site-1"
                       :effect :propose :patch {:volume-m3 420 :moisture-pct 32 :shift "day"}} op-p1)
    (approve! actor "t1")
    (exec! actor "t2" {:op :log-extraction-record :site-id "peat-site-1"
                       :effect :propose :patch {:volume-m3 450 :moisture-pct 30 :shift "night"}} op-p3)
    (exec! actor "t3" {:op :schedule-extraction-operation :site-id "peat-site-2"
                       :effect :propose :patch {:activity "sod-cutting" :window "2026-07-20"}} op-p3)
    (exec! actor "t4" {:op :flag-environmental-concern :site-id "peat-site-2"
                       :effect :propose :patch {:concern "elevated-runoff-near-drainage" :confidence 0.95}} op-p3)
    (approve! actor "t4")
    (exec! actor "t5" {:op :log-extraction-record :site-id "peat-site-9"
                       :effect :propose :patch {:volume-m3 100 :moisture-pct 40 :shift "day"}} op-p3)
    db))
...
(def ^:private gate-rows
  ["        <tr><td><code>:log-extraction-record</code></td><td><span class=\"warn\">phase-1 always approval; phase-3 auto-commit when clean</span></td></tr>"
   "        <tr><td><code>:flag-environmental-concern</code></td><td><span class=\"warn\">ALWAYS human approval (environmental safety)</span></td></tr>"
   "        <tr><td><code>:schedule-extraction-operation</code></td><td><span class=\"warn\">registered + verified site required</span></td></tr>"
   "        <tr><td><code>:coordinate-shipment</code></td><td><span class=\"warn\">ALWAYS human approval (actuation)</span></td></tr>"])
(defn render [db]
  (let [ledger (vec (store/ledger db))
        sites (->> (store/all-sites db) (sort-by (juxt :id)))
        srow (fn [s] (format "        <tr><td>%s</td><td>%s</td><td>%s</td></tr>" (esc (:id s)) (esc (or (:status s) "-")) (status-cell ledger (:id s))))
        srows (str/join "\n" (map srow sites))
        lrows (str/join "\n" (map ledger-row ledger))]
    (str "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-0892</title>"
     ...
     "<header class=\"bar\"><h1>Peat extraction ops (ISIC 0892) — <code>peatops</code></h1></header><main>"
     "<section class=\"card\"><h2>Peat sites</h2>"
     "<p class=\"muted\">Demo from <code>peatops.store</code> via <code>peatops.render-html</code>. No invented data.</p>"
