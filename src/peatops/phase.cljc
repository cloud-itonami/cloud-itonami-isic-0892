(ns peatops.phase
  "Phase 0->3 staged rollout for the ISIC-0892 peat-extraction
  site-operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-extraction  -- extraction-record logging
                                     allowed, every write needs human
                                     approval.
    Phase 2  assisted-coordination -- adds extraction-operation
                                     scheduling and shipment coordination
                                     writes, still approval.
    Phase 3  supervised auto      -- governor-clean, high-confidence
                                     `:log-extraction-record`/
                                     `:schedule-extraction-operation`/
                                     `:coordinate-shipment` may
                                     auto-commit. `:flag-environmental-
  ...
  `peatops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [peatops.governor :as governor]))
...
(defn verdict->disposition
  "Map a PeatExtractionGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
